...
export function ChatWidget({ open, onClose }: ChatWidgetProps) {
  ...
  useEffect(() => {
    ...
    api.datasets
      .list(session.access_token, targetOrgId)
      .then((d) => {
        setDatasets(d)
        if (d.length > 0 && !activeDatasetId) setActiveDataset(d[0].id)
      })
      .catch(() => {})
  }, [session, open, organizationId, user?.role, activeDatasetId, setActiveDataset])
  ...
  async function submitMessage(rawMessage: string) {
    ...
    if (!thread) {
      const datasetId = activeDatasetId ?? datasets[0]?.id
      if (!datasetId) {
                ...
                >
                  ...
                  {!hasDataset && showWelcome && (
                    <p className="text-xs text-center text-[#a09e99] mt-1">
                      Upload a dataset to ask data-specific questions.
                    </p>
                  )}
                ...
                <div className="border-t border-[#ebebeb] bg-[#fff9ef] px-4 py-3">
                  <form onSubmit={sendMessage} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      disabled={streaming || insightLoading}
                      maxLength={500}
                      placeholder={insightLoading ? 'Generating insight…' : 'Ask a question…'}
                    ...
                    {streaming ? (
                      ...
                      <button
                        type="submit"
                        disabled={!input.trim() || insightLoading}
